...
```{r}
df %>% 
  select(crash_year, crash_month, person_count, fatal_count, injury_count, unb_death_count, belted_death_count, bicycle_maj_inj_count, ped_maj_inj_count, ped_death_count, bicycle_maj_inj_count, bicycle_death_count) %>% 
  arrange(crash_year, crash_month) %>%
  mutate(crash_year = fct_inorder(as.character(crash_year)),
         time_period = str_c(crash_year, crash_month, sep = "-"),
         time_period = fct_inorder(time_period)) %>%
...
df_cumulative %>% 
  ggplot(aes(time_period, n_cumulative, group = measure, color = measure)) +
  geom_line(size = 1) +
  facet_wrap(~measure,
             scales = "free_y") +
  scale_color_viridis(discrete = TRUE) +
