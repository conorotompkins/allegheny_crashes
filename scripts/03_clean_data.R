...
df_combined_allegheny_county_crash_data_2004_2017_raw %>% 
  ...
  mutate(automobile = case_when(automobile_count >= 1 ~ TRUE,
         ...
         pedestrian = case_when(ped_count >= 1 ~ TRUE,
                                ped_count < 1 ~ FALSE),
         speeding_related = as.integer(as.logical(speeding_related)),
         dec_lat = as.integer(dec_lat),
         dec_long = as.integer(dec_long),
         est_hrs_closed = as.integer(est_hrs_closed),
         lane_closed = as.integer(lane_closed),
         cons_zone_spd_lim = as.integer(cons_zone_spd_lim),
         workers_pres = as.logical(as.integer(workers_pres)),
         fatalities_per_person = fatal_count / person_count) %>% 
  mutate_at(vars(interstate:trolley), as.logical) %>%  
  filter(hour_of_day <= 23) -> data
glimpse(data)