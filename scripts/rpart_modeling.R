...
df <- read_csv("data/df_2013_cleaned.csv")

df %>% 
  select(drinking_driver:cross_median) %>% 
  str()
...
df %>% 
  mutate(crash_year = as.integer(as.double(crash_year)),
         ...
         motorcycle = as.logical(motorcycle),
         bicycle = as.logical(bicycle),
         pedestrian = as.logical(pedestrian)) %>% 
...
df %>% 
  select(fatal, crash_year, day_of_week, hour_of_day, weather, illumination, road_condition, collision_type, relation_to_road, urban_rural, person_count, automobile_count, motorcycle_count, bus_count, small_truck_count, heavy_truck_count, suv_count, van_count, bicycle_count, ped_count, speed_limit, contains("driver"),
  ...
  mutate_at(vars(drinking_driver:cross_median), as.logical) %>%
...
df_model_1 %>% 
  ...
  rename(pred_non_fatal = `FALSE`,
         pred_fatal = `TRUE`) %>% 
  select(fatal, pred_fatal, pred_non_fatal, everything()) -> df_model_1_rpart
df_model_1_rpart
prp(model_1)
tidy(model_1)
summary(model_1)
...
df %>% 
  select(fatal_count, injury_count, crash_year, day_of_week, hour_of_day, weather, illumination, road_condition, collision_type, relation_to_road, urban_rural, person_count, automobile_count, motorcycle_count, bus, small_truck_count, heavy_truck_count, suv_count, van_count, bicycle_count, ped_count, speed_limit, contains("driver"),
         speeding, cell_phone, alcohol_related, drinking_driver, aggressive_driving, fatigue_asleep, running_red_lt, running_stop_sign, tailgating, cross_median, impaired_driver)  %>% 
  mutate_at(vars(drinking_driver:cross_median), as.logical) %>%
...
df_model_2 %>% 
  ...
  select(casualties, .pred, .resid, everything()) -> df_model_2_rpart
df_model_2_rpart
prp(model_2)
tidy(model_2)
summary(model_2)
